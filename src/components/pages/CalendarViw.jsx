...
import { TasksAPI, LogsAPI, JobsAPI } from "../db/database";
...
export default function CalendarView() {
  ...
  const loadData = async () => {
    setLoading(true);
    const [t, l, j] = await Promise.all([TasksAPI.getAll(), LogsAPI.getAll(), JobsAPI.getAll()]);
    setTasks(t);
    setLogs(l);
    setJobs(j);
    setLoading(false);
  ...
  const selectedDateLogs = selectedDate ? logs.filter((l) => isSameDay(new Date(l.date), selectedDate)) : [];
      ...
      >
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className={`text-2xl font-bold ${darkMode ? "text-white" : "text-gray-900"}`}>Calendar</h1>
          </div>
        </div>
      </header>
          ...
          <div className="space-y-4">
            {selectedDate ? (
              <>
                ...
                <Card className="dark:bg-gray-800 dark:border-gray-700">
                  <CardHeader>
                    <CardTitle className="text-lg dark:text-white">Tasks ({selectedDateTasks.length})</CardTitle>
                  </CardHeader>
                  <CardContent>
                        ...
                        {selectedDateTasks.map((task) => (
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`font-medium ${darkMode ? "text-white" : ""}`}>{task.title}</span>
                            </div>
                ...
                <Card className="dark:bg-gray-800 dark:border-gray-700">
                  <CardHeader>
                    <CardTitle className="text-lg dark:text-white">Daily Logs ({selectedDateLogs.length})</CardTitle>
                  </CardHeader>
                  <CardContent>
                    {selectedDateLogs.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {selectedDateLogs.map((log) => (
                          ...
                          >
                            <div className={`font-medium ${darkMode ? "text-white" : ""}`}>{getJobName(log.jobId)}</div>
              ...
              <Card className="dark:bg-gray-800 dark:border-gray-700">
                <CardContent className="py-12 text-center">
                  <p className={`${darkMode ? "text-gray-400" : "text-gray-600"}`}>Click on a date to see details</p>
                </CardContent>
              </Card>
            )}
          </div>
